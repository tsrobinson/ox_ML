...
Suppose we have two alternative values of $\bm{\theta}$: $\bm{\theta_1}, \bm{\theta_2}$. We can calculate the likelihood *ratio* (LR) of these two possible parameter values:
$$
LR = \frac{\mathcal{L}(\bm{\theta_1})}{\mathcal{L}(\bm{\theta_2)}}
$$
...
* If $y_i = 1$, we want the $f_{\bm{\theta},b}(\mathbf{x_i})$

* But if $y_i = 0$ we want the inverse, i.e. $(1-f_{\bm{\theta},b}(\mathbf{x_i}))$

* We can combine these two using a mathematical "logic gate":
$$
\mathcal{L}_{\bm{\theta},b} = f_{\bm{\theta}}(\mathbf{X})^{\bm{y}} \times (1-f_{\bm{\theta}}(\mathbf{X}))^{(1-\bm{y})},
$$
  since when $y_i = 0, x^{y_i} = 1$ and $x^{(1-y_i)} = x$, and vice versa.
  
Simplifying, since $f_{\bm{\theta},b}(\bm{x_i}) = \hat{y}_i$:
$$
