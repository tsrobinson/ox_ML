...
* $\operatorname*{arg\,min}_{g} |f(\bm{X}) - \hat{g}_{\lambda,\theta}(\bm{X})|$
    * Impossible to solve in most cases
...
Choosing a *single* best model may still be suboptimal:

* Different estimating strategies may be better at modeling different parts of the outcome

The alternative is to combine models:

* We can average models together
* We can adapt models iteratively
...
$$
S_L(\bm{X}) = \sum_{l=1}^{L}c_l \times w_l(\bm{X}),
$$
where:

* $L$ indexes a series of weak learners
* $c_l$ is a coefficient corresponding to the $l$th weak learner $w_l$

To optimise $\bm{c}$ and $\bm{w}$, we add the models sequentially to $S_L$

The optimisation function at each iteration is defined as:
$$
\operatorname*{arg\,min}_{c_l,w_l} \sum_{i=1}^{N}e(y_i, s_{l-1}(x_i) + c_l \times w_l(x_i))
$$
...
Then we estimate an OLS model:
$$
y_i = \sum_{m=1}^Mw_m\hat{y}_{im} + \epsilon_i.
$$
So that $w_m$ are weights, we impose two further restrictions on $\bm{w}$:

* $w_m \geq 0$
* $\sum_m{w_m} = 1$

Denote the resultant trained model $\hat{H}$
...
2. Pass the resultant predictions through $\hat{H}$
    ...
    * I.e. $\hat{\bm{y'}} = \hat{H}(\bm{X'}) = \hat{\alpha} + \sum_{m=1}^M{\hat{w}_m\hat{\bm{y'}_m}}$
...
## Application to heterogeneous effects prediction
