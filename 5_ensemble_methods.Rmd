...
To optimise $\bm{c}$ and $\bm{w} = \{w_1,\cdots,w_L\}$, we add the models sequentially to $S_L$, *reweighting* the data after each model

The optimisation function at each iteration is defined as:
$$
\operatorname*{arg\,min}_{c_l,w_l} \sum_{i=1}^{N}e(y_i, s_{l-1}(\bm{x_i}) + c_l \times w_l(\bm{x_i}^{(l)}))
$$
